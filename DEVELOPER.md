...
Run `DOCKER_IMAGE=sonarscanner-cli ./docker-sonarscanner-builder.sh --no-push` to build and tag every supported image locally without publishing image tags.

The `Test SonarScanner Image` workflow runs this command for every push on master and pull request.

Publishing and master CI use a separate Buildx registry cache for each .NET version at `${GHCR_IMAGE}:buildcache-master-net<version>`.

Same-repository pull requests read the master cache and write to `${GHCR_IMAGE}:buildcache-pr-<number>-net<version>`. Fork pull requests do not access registry caches.

The `--pull` option still checks for updated base images on every scheduled run.
